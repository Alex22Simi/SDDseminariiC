...
void afisareMatrice(float** mat, char** denumiri, int nr) {
    for (int i = 0; i < nr; i++) {
        printf("\nCod = %.0f, Denumire = %s, Pret = %5.2f, Cantitate = %5.2f",
...
void dezalocareMatrice(float** mat, char** denumiri, int nr) {
    for (int i = 0; i < nr; i++) {
        free(mat[i]);
        free(denumiri[i]);  // Adăugat pentru a evita memory leaks
    }
    free(mat);
    free(denumiri);
}
...
void main() {
    ...
    // Alocare matrice și vector de denumiri
    float** mat = (float**)malloc(nrProd * sizeof(float*));
    for (int i = 0; i < nrProd; i++) {
        mat[i] = (float*)malloc(3 * sizeof(float));
    }
