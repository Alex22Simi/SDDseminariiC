...
nodLS* initializareNod(student s)
{
	nodLS* nou = (nodLS*)malloc(sizeof(nodLS)); //alocam spatiul de 16 b
	//nou->info = s; //shallow copy
	nou->info.varsta = (int*);
	nou->info.nume = (char*)malloc((strlen(s.nume) + 1) * sizeof(char));
	strcpy(nou->info.nume, s.nume);
...
nodLS* inserareNod1(nodLS* capLS, student s) {
	...
	else //deplasare secventiala in lissta cu o alta variabila temp
		//ca sa o deplasam sa trecem prin toate nodurile listei mele ca sa ajung la ultimul nod nenul
	{
		nodLS* temp = capLS;
		while (temp->next != NULL)
		{
...
void traversareLista(nodLS* capLS)
{
	...
	while (temp != NULL)
	{
		printf("\nVarsta = %d, Nume = %s, Medie = %5.2f", temp->info.varsta, temp->info.nume, temp->info.medie);
		temp = temp->next;
	}
}
...
void dezalocareLista(nodLS* capLS)
{
	...
	while (temp != NULL)
	{
		...
		free(temp->info.nume);
		free(temp);
		//cate mallocuri avem atatea free trebuie sa avem
		temp = aux;
	}
}
...
void salvareStudentiVector(nodLS* capLS, student* vect, int* nrElem, float mediePrag)
{
	...
	while (temp != NULL)
	{
		if (temp->info.medie >= mediePrag)
		{
			...
			vect[*nrElem].varsta = (int*)malloc(sizeof(int));
			*(vect[*nrElem].varsta) = *(temp->info.varsta);
			vect[*nrElem].nume = (char*)malloc((strlen(temp->info.nume)))
				strcpy(vect[*nrElem].nume, temp->info.nume);
