...
typedef struct
{
	int BF; //indicator balance factor
	zbor inf;
	...
	if (aux->dreapta != NULL) {
		...
		if (aux->stanga != NULL) {
			...
			while (temp->stanga != NULL) {
				temp = temp->stanga;//cel mai mare subarbore din arborele stang ->se duce pe ultimul nod din dreapta
			}
			temp->stanga = aux->stanga; //pe dreapta ultimului nod din stanga leg ce era inainte pe dreapta
		}
	}
	else {
		if (aux->stanga != NULL) {
	...
	if (rad != NULL) {
		if (rad->inf.cod == cheieDeSters) {
			...
			if (cheieDeSters < rad->inf.cod) {
				...
				rad->dreapta = stergereRad(rad->dreapta, cheieDeSters);
				return rad;
			}
		}
	}
...
//schimbam mereu fisierul pentru a putea testa