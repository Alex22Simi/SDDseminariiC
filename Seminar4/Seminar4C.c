...
typedef struct {
	int zi, luna, an; //12B in structura dataReceptioner
} dataReceptionare;
...
typedef struct {
	...
	float durata; //4B
} notificare; //20B are strutura

typedef struct {
	notificare info; //20B
	struct nodStiva* next; //4B
} nodStiva; //24 B are structura

typedef struct {
	notificare info; //20B
	struct nodCoada* next; //4B
} nodCoada; //24 B are structura

//fct de inserare nod in stiva
//ar fi bine sa pastram aceasta sintaxa
...
void afisareStiva(nodStiva* varf) {
	...
	while (temp) {
		printf("\nData: %d/%d/%d, Denumire aplicatie: %s, Durata: %5.2f",
			temp->info.data.zi, temp->info.data.luna, temp->info.data.an, temp->info.denumireAplicatie,
			temp->info.durata);
		temp = temp->next;
	}
}
...
int get(nodCoada** prim, nodCoada** ultim, notificare* notif) {
	...
	//cand s a sters ultimul nod din coada
	if (*prim == NULL) {
		*ultim = NULL;
		return -1;
	}
...
void main() {
	...
	for (int i = 0; i < nr; i++) {
		printf("\nData: %d/%d/%d, Denumire aplicatie: %s, Durata: %5.2f",
			vect[i].data.zi, vect[i].data.luna, vect[i].data.an, vect[i].denumireAplicatie,
			vect[i].durata);
	}
	for (int i = 0; i < nr; i++) {
