//arbore binar de cautare -> nu exista chei dublicate
//in fisier sa nu punem toate codurile crescatoare deoarece toate o sa se duca in dreapta
//trebuie sa fie valori mixate
/*#include<malloc.h>
#include<stdio.h>
...
//RSD -> radacina stanga dreapta -> pre ordine
...
//SRD -> stanga radacina dreapta -> in ordine (afiseaza cheile sortate crescator
...
//SDR -> stanta dreapta radacina -> post ordine
...
//functie sa se salveze intr un vector zborurile din anul 2023
//aflate in nodurile frunza ale arborului
...
void main() {
	...
	for (int i = 0; i < nr; i++) {
		printf("\nCod = %d, Companie = %s, Pret = %5.2f, Data = %d/%d/%d",
			vect[i].cod, vect[i].companie, vect[i].pret, vect[i].dataZbor.zi, vect[i].dataZbor.luna,
			vect[i].dataZbor.an);
	}
	for (int i = 0; i < nr; i++) {
